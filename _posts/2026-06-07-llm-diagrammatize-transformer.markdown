...
1. （多头自注意力 → 残差连接 & 层归一化）
   - 原则：编码器是双向的，每个位置在计算自注意力时，可以看到整个输入序列中的所有位置（包括左边和右边的词），没有”未来”的概念。
   - 掩码：仅使用 填充掩码（Padding Mask），忽略句子中的 [PAD] 占位符，不涉及后续掩码（Causal Mask）。
...
- 原则：解码器采用自回归生成方式，每个位置只能看到当前位置及之前的位置，不能看到未来 token。
...
注意由于解码器里有两处注意力，因此对应的也有两处掩码：  
1. 解码器自注意力掩码：用于遮挡解码器自身的填充位(decoder padding token)和未来token(causal constraint)，在解码器的多头自注意力里使用  
2. 解码器-编码器注意力掩码: 用于遮挡编码器的填充位(encoder padding token)，在解码器-编码器的交叉注意力里使用。注意 cross-attention 不涉及 causal mask，因为 encoder 是完整可见序列。   

粗略可以得出：Decoder = Encoder + Cross-Attention + Causal Mask  