...
官网主推的自然是第二种，我在使用过程中也是发现二的功能更强大，一则适合我们理解记忆处理的过程。

### 2.1. oss

我们重点介绍 add search 两个接口是如何工作的，使用的是 chroma 向量库。

#### 2.1.1. Add Memory

![add_architecture](/assets/images/mem0/add_architecture.avif)

实际代码，由于写入 Vector 与 Graph 要提取的结构化数据不同，实际是分别调用的 LLM，也就是在 LLM 之前，流程就分为两道了。
...
可以看到平台相比于开源，多了 Categories、筛选等能力。完全依据向量库，做筛选、排序还是比较费劲的，猜测平台版本可能用了向量库+关系数据库双写的方式。

Mem0 Platform 支持了 Conversation、Session、User、Organizational Memory , Organizational 则是为了在多人间共享记忆所做的探索。
...
现在看来，记忆已经是智能体的核心之一。但是整体还是偏供给侧，即作为基础设施提供出来，上层按照需求规划如何使用。

但我觉得记忆应该更前一步，作为需求侧的核心设施存在，因为无论未来是单个智能体的多个会话、多个智能体之间，都会存在记忆共享的需求。我对于的记忆的理解，是能够**有办法沉淀下来，成为自己的自定义的 SKILL，然后在多个使用场景里按需加载**。

比如我最近看算法，在思考二分、后缀数组等问题，而之前则对 LevelDB 里的 SkipList、BloomFilter 研究较多，我就会希望将自己原来的理解加载到上下文里，再去咨询 LLM 关于后缀数组怎么优化的问题。这些 LevelDB 里关于算法的理解的记忆，我希望能将其作为我定义的的 SKILL. 这点是通用的 SKILL 无法满足的。
