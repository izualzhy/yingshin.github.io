...
我的理解就是**通过相似度进一步计算出来了注意力(增加了softmax)，作为 X1 的新表示**。
...
第 3 步： attn_weights.shape = (2, 3, 5), 矩阵表示 x1 x2 之间两两 token 的关注程度，其中一行表示x1的 1 个 token 对 x2 所有 token 的关注程度
例如：
```python
attn_weights =
[
...
2. decoder RNN层计算出 output 后，继续计算综合 encoder_output 的 attention contex 及 attention weight 
