...
更合理的智能体框架形式，或许是类似 Apache Flink 的做法，框架实现主体逻辑，对接各类存储的工作则放到 connector 实现，使用时按需加载。而 Flink 能做到这点，是基于足够多的应用场景，抽象出来了流处理的各个阶段及定义。

这一点 ADK 和其他 Agent 框架一样，面临着相同的局面。框架提供了很多佐料、食材，但是好吃的菜却凤毛麟角。就好像当前好用的 Agent 少之又少，也就无法支持框架达到新的阶段了。
...
**Session 通过 SessionService 管理**，持 `InMemory`、`Database` (MySQL 5 表模式) 及 `VertexAi` 后端
...
Event 用来驱动整个过程，例如提问、回答、记忆压缩、function_call，都是不同的 Event，当我们不使用`adk web`而是自行管理 SessionService 时，就需要处理 Event 了。
...
| user_id | state (JSON) | update_time |
| :--- | :--- | :--- |
| user1 | `{"color": "红色", "food": "火锅"}` | 2026-04-11 18:12 |
| user2 | `{"color": "蓝色", "food": "烧烤"}` | 2026-04-11 18:13 |
...
1. **AgentTeam VS Workflow:** 在 Agent Team 与 Workflow 之间，边界在于“确定性”：Workflow 提供路径约束，而单个 Agent 内部依然保持生成式的灵活性, 当前有三类简洁的 flow 形式
